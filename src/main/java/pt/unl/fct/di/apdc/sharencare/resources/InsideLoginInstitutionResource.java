...
//			return Response.status(Status.BAD_REQUEST).entity("Invalid data").build();
		...
		user = Entity.newBuilder(userKey).set("username", user.getString("username"))
				...
				.set("profilePic", "").set("landLine", landLine).set("mobile", mobile).set("address", address)
...
//		return Response.status(Status.BAD_REQUEST).entity("Invalid data").build();
		...
		user = Entity.newBuilder(userKey).set("username", token.getString("username"))
				.set("password", user.getString("password")).set("nif", user.getString("nif"))
				.set("profilePic", user.getString("profilePic")).set("email", email).set("landLine", landLine)
				.set("mobile", mobile).set("address", address).set("postal", zipCode).set("website", website)
				.set("twitter", twitter).set("instagram", instagram).set("youtube", youtube).set("facebook", facebook)
				.set("fax", fax)
			...
			if (data.validPasswordLength()) {
				if (data.newPassword.equals(data.confirmation)) {
					user = Entity.newBuilder(userKey).set("nif", user.getString("nif"))
							...
							.set("address", user.getString("address")).set("postal", user.getString("postal"))
							...
							.set("members", user.getString("members")).set("events", user.getString("events")).build();
					datastore.put(user);
