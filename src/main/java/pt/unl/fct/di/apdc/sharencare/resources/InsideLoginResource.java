...
//					.build();
    ...
    private boolean validateData(ChangePropertyData data) {
        ...
        if (data.newEmail.contains("@") && (email[emailSize].length() == 2 || email[emailSize].length() == 3))
            if (data.newProfileType.equals("") || data.newProfileType.equalsIgnoreCase("Publico") || data.newProfileType.equalsIgnoreCase("Privado"))
                if (data.newLandLine.equals("") || (landLine[0].subSequence(0, 1).equals("+") && landLine[1].length() == 9))
