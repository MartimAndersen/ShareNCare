		...
		try {
			...
			if (event != null) {
				...
				user = Entity.newBuilder(userKey).set("nif", token.getString("username"))
						...
						.set("state", user.getString("state")).set("coordinates", user.getString("coordinates"))
						.build();
...
//        }
		...
		institution = Entity.newBuilder(institutionKey).set("nif", token.getString("username"))
				...
				.set("state", institution.getString("state")).set("coordinates", institution.getString("coordinates"))
				.build();
	...
	public Response getAllEvents(@CookieParam("Token") NewCookie cookie) {
		...
		while (eventsQuery.hasNext()) {
			...
			String finalDate = e.getString("ending_date");
			String eventName = e.getString("name");
			handleFinishEvent(events, e, finalDate, eventName);
		}

		return Response.ok(g.toJson(events)).cookie(cookie).build();
	}

	private void handleFinishEvent(List<String> events, Entity e, String finalDate, String eventName) {
		if (!hasEnded(finalDate)) {
			String event = g.toJson(e.getProperties().values());
			events.add(event);
		} else {
			String ended = e.getString("ended");
			if (ended.equals("false")) {
				Key eventKey = datastore.newKeyFactory().setKind("Event").newKey(eventName);
				Entity event = datastore.get(eventKey);
				handlePointsDistribution(event);
				updateEndedAttribute(event, eventKey);
			}
		}
	}
	...
	private boolean isBefore(String date1, String date2) {
		...
		if (Integer.parseInt(dateSecond[2]) >= Integer.parseInt(dateFirst[2])) {

			if (Integer.parseInt(dateSecond[1]) >= Integer.parseInt(dateFirst[1])) {

				if (Integer.parseInt(dateSecond[0]) >= Integer.parseInt(dateFirst[0])) {
					return true;
				}
			}
		}
	...
	private void handlePointsDistribution(Entity event) {
		String m = event.getString("members");
	...
	private void updateEndedAttribute(Entity event, Key eventKey) {
		...
		event = Entity.newBuilder(eventKey).set("name", event.getString("name"))
				...
				.set("tags", event.getString("tags")).set("rating", event.getString("rating")).set("ended", "true")
				.build();
	...
	private boolean is3DaysBefore(String date) {
		...
		Date currDate = new GregorianCalendar(Integer.parseInt(curr[2]), Integer.parseInt(curr[1]) - 1,
				Integer.parseInt(curr[0])).getTime();
		Date initialDate = new GregorianCalendar(Integer.parseInt(datefinal[2]), Integer.parseInt(datefinal[1]) - 1,
				Integer.parseInt(datefinal[0])).getTime();
	...
	public Response editEvent(@CookieParam("Token") NewCookie cookie, EditEventData data) throws ParseException {
		...
		event = Entity.newBuilder(eventKey).set("name", event.getString("name")).set("description", description)
				.set("minParticipants", minParticipants).set("maxParticipants", maxParticipants).set("time", time)
				.set("coordinates", coordinates).set("durability", durability)
				.set("institutionName", event.getString("institutionName")).set("initial_date", initialDate)
				.set("ending_date", endingDate).set("members", event.getString("members"))
				.set("points", event.getString("points")).set("tags", tags1).set("rating", event.getString("rating"))
				.set("ended", event.getString("ended")).build();
